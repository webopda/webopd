  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">Hapus User {{$item->name}}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{ url('admin/user/hapus') .'/'. $item->id }}" enctype="multipart/form-data" method="POST">
            @csrf
            @method('delete')
  <div class="form-group">
     Apakah Yakin ingin Menghapus Data User {{ $item->name }} ?
      </div>
