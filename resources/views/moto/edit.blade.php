    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit moto {{$item->moto}} </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
<form action="{{ url('admin/moto/update' .'/'. $item->id) }}" method="post">
    @csrf
@method('put')
                <div class="mb-3">
                    <label for="moto" class="form-label">moto</label>
                    <input type="text" class="form-control" id="moto" name="nama_kategori" placeholder="Enter moto" value="{{$item->moto}}" required>
                    @error('moto')
