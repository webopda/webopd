    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit visi {{$item->visi}} </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
<form action="{{ url('admin/visi/update' .'/'. $item->id) }}" method="post">
    @csrf
@method('put')
                <div class="mb-3">
                    <label for="visi" class="form-label">visi</label>
                    <input type="text" class="form-control" id="visi" name="nama_kategori" placeholder="Enter visi" value="{{$item->visi}}" required>
                    @error('visi')
