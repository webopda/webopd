...
<div class="content-wrapper">
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Daftar Rawat Jalan</h4>
                <div class="col-sm-2 pt-6">
                    <button type="button" class="btn btn-block btn-primary" id="btnAddRawatJalan"> 
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#btnAddRawatJalan').click(function () {
            $.get("{{ url('rawatjalan/create') }}", function (res) {
        ...
        window.confirmDelete = function(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('rawatjalan.destroy', '') }}/" + id,
                        type: 'POST',
                        data: {
                            _method: 'DELETE',
                            _token: '{{ csrf_token() }}',
        ...
        window.openEditModal = function(id) {
            $.get("{{ url('rawatjalan') }}/" + id + "/edit", function(res) {
                let options = '';
                res.dokters.forEach(d => {
                    options += `<option value="${d.id}" ${d.id == res.rawatJalan.dokter_id ? 'selected' : ''}>${d.nama}</option>`;
                });

                let formHtml = `
                    <form id="editFormSwal">
                        @csrf
                        <input type="hidden" name="id" value="${res.rawatJalan.id}">
                        <div class="form-group text-left">
                            <label>Nama Dokter</label>
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ url('rawatjalan') }}/" + res.rawatJalan.id + "/update",
                            method: "POST",
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
