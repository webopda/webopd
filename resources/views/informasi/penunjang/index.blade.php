...
<div class="content-wrapper">
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Daftar Penunjang</h4>
                <div class="col-sm-2 pt-6">
                    <a href="{{ route('penunjang.create') }}" type="button" class="btn btn-block btn-primary"> 
...
<script type="text/javascript">
    $(document).ready(function() {
        var table = $('.data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('penunjang.index') }}",
            scrollX: true,
            autoWidth: false, 
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex', class: 'text-center', orderable: false, searchable: false},
                {data: 'penunjang', name: 'penunjang', class: 'text-center'}, 
        ...
        function deleteData(id) {
            $.ajax({
                url: "{{ route('penunjang.destroy', '') }}/" + id,
                type: 'POST',
                data: {
                    _method: 'DELETE',
                    _token: '{{ csrf_token() }}',
